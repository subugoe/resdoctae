...
<xsl:stylesheet
	...
	<xsl:if test="($auth != 'no') or ($dim/dim:field[@element='format'][@qualifier='extent'][1] != '0')">
	    <xref target="{@OBJID}" n="community-browser-link">
	    <!-- hide sortmark in title -->		
	    <xsl:choose>
		    <xsl:when test="contains($dim/dim:field[@element='title'], '# ')">
			    <xsl:value-of select="substring-after($dim/dim:field[@element='title'], '# ')"/>
		    </xsl:when>
		    <xsl:otherwise>

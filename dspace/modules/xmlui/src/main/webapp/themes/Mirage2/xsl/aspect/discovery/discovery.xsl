...
-->
    ...
    <xsl:template name="itemSummaryList">
	  ...
	  <xsl:choose>
            <xsl:when test="dri:list[@n=(concat($handle, ':dc.type'))] = 'biography'">
		<div class="col-xs-12">
                            <div class="bold">
                                    <xsl:element name="a">
					...
					<xsl:value-of select="dri:list[@n=(concat($handle, ':dc.title'))] "/>
                                     </xsl:element>
                                </div>
                            <xsl:if test="dri:list[@n=(concat($handle, ':dc.date.birthday'))]">
                                    <div><xsl:value-of select="translate(dri:list[@n=(concat($handle, ':dc.date.birthday'))], '-', '.')" /><xsl:text> - </xsl:text><xsl:value-of select="translate(dri:list[@n=(concat($handle, ':dc.date.deathday'))], '-', '.')" /></div>
                           </xsl:if>
		</div>
	    </xsl:when>
	    <xsl:otherwise>
		...
		<xsl:if test="dri:list[@n=(concat($handle, ':dc.relation.ispartof'))]">
                        <xsl:choose>
                            <xsl:when test="starts-with(dri:list[@n=(concat($handle, ':dc.relation.ispartof'))], 'http')">
                                <xsl:variable name="handle"><xsl:value-of select="substring-after(dri:list[@n=(concat($handle, ':dc.relation.ispartof'))], 'hdl.handle.net')"/></xsl:variable>
                                <xsl:variable name="metsfile"><xsl:value-of select="concat('cocoon://metadata/handle',$handle, '/mets.xml')"/></xsl:variable>
                                <a>
                                    <xsl:attribute name="href"><xsl:value-of select="dri:list[@n=(concat($handle, ':dc.relation.ispartof'))]"/></xsl:attribute>
                                    <xsl:value-of select="document($metsfile)//dim:field[@element='title']"/>
                                </a>
                            </xsl:when>
                            <xsl:otherwise>

                                <xsl:value-of select="dri:list[@n=(concat($handle, ':dc.relation.ispartof'))]"/>
                            </xsl:otherwise>
                        </xsl:choose>
                </xsl:if>

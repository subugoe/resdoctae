...
<xsl:stylesheet
	...
	<!-- Do not show empty containers for unauthenticated user -->
        <xsl:if test="($data/dim:field[@element='format'][@qualifier='extent'][1] != 0) or (//dri:userMeta/@authenticated = 'yes')">
    ...
    <xsl:template match="dri:field[@rend = 'community-browser-toggle-button']">
	   <!-- Do not show empty communities / collections for unauthenticated user -->
	   <xsl:if test="count(../../dri:div/dri:xref) &gt; 0">
